...
const main = async () => {
  ...
  const [teams, members, clients, cases, documents, invalidOwnerTeams, activeTeams, activeOwners, orphanCases, orphanDocuments] = await Promise.all([
  ...
  const result = { teams, members, clients, cases, documents, invalidOwnerTeams: invalidOwnerTeams.length, ownerMismatches: ownerMismatches.length, orphanCases: orphanCases.length, orphanDocuments: orphanDocuments.length };
  console.log(JSON.stringify(result));
  if (result.invalidOwnerTeams || result.ownerMismatches || result.orphanCases || result.orphanDocuments) process.exitCode = 1;
};
